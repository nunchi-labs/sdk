...
/// Mempool ingress required by the oracle RPC server's write surface.
#[async_trait]
...
/// Concrete oracle mempool RPC server over an ingress backend.
#[derive(Clone)]
pub struct OracleMempoolRpc<I> {
    ingress: I,
}

impl<I> OracleMempoolRpc<I> {
...
#[async_trait]
impl<I> OracleMempoolServer for OracleMempoolRpc<I>
where
    I: MempoolIngress,
{
    async fn submit_transaction(
        ...
        let transaction: Transaction = decode_hex(&transaction, "oracle transaction")?;
        let hash = self
            .ingress
            .submit(transaction)
            .await
...
/// Client-side parameter struct for `oracle.submit_transaction`.
...
/// Register the oracle module's mempool RPC methods into a downstream router.
pub fn register_mempool<Context, I>(
    router: &mut RpcRouter<Context>,
    rpc: OracleMempoolRpc<I>,
...
mod tests {
    ...
    use super::*;
    use crate::{FeedId, FeedPayload, OracleOperation};
    ...
    fn module(ingress: MockIngress) -> jsonrpsee::RpcModule<()> {
        ...
        register_mempool(&mut router, OracleMempoolRpc::new(ingress))
            .expect("register oracle mempool RPC");
        router.into_module()
    }
    ...
    fn submit_transaction_returns_hash() {
        commonware_runtime::deterministic::Runner::default().start(|_| async move {
            ...
            let response: SubmitTransactionResponse = module
                .call("oracle.submit_transaction", params)
                .await
    ...
    fn submit_transaction_maps_admission_errors() {
        commonware_runtime::deterministic::Runner::default().start(|_| async move {
            ...
            params
                ...
                .expect("serialize transaction param");
            let error = module
                .call::<_, SubmitTransactionResponse>("oracle.submit_transaction", params)
                .await
    ...
    fn transaction_status_reports_lifecycle() {
        commonware_runtime::deterministic::Runner::default().start(|_| async move {
            ...
            let response: TransactionStatusResponse = module
                .call("oracle.transaction_status", params)
                .await
            ...
            unknown_params
                ...
                .expect("serialize hash param");
            let response: TransactionStatusResponse = module
                .call("oracle.transaction_status", unknown_params)
                .await
                .expect("status response");
