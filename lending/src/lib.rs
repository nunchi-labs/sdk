...
//! collateral-factor market with explicit fixed and ISFR-floating rate quotes.
...
/// Fixed and floating APR terms for a market, expressed in basis points.
///
/// Floating APR is quoted from the live benchmark as `ISFR + floating_spread_bps`.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct InterestRateTerms {
    pub benchmark: RateBenchmark,
    pub fixed_rate_bps: i64,
    pub floating_spread_bps: i64,
}

impl InterestRateTerms {
    pub fn isfr(fixed_rate_bps: i64, floating_spread_bps: i64) -> Self {
        Self {
    ...
    pub fn floating_rate_bps(&self, benchmark_rate_bps: i64) -> Result<i64, LendingError> {
        benchmark_rate_bps
            .checked_add(self.floating_spread_bps)
            .ok_or(LendingError::RateOverflow)
    }
}

impl Write for InterestRateTerms {
    fn write(&self, buf: &mut impl bytes::BufMut) {
        self.benchmark.write(buf);
        self.fixed_rate_bps.write(buf);
        self.floating_spread_bps.write(buf);
    }
}

impl Read for InterestRateTerms {
    type Cfg = ();

    fn read_cfg(buf: &mut impl bytes::Buf, _: &Self::Cfg) -> Result<Self, CodecError> {
        Ok(Self {
            benchmark: RateBenchmark::read(buf)?,
            fixed_rate_bps: i64::read(buf)?,
            floating_spread_bps: i64::read(buf)?,
        })
    }
}

impl EncodeSize for InterestRateTerms {
    fn encode_size(&self) -> usize {
        self.benchmark.encode_size()
            + self.fixed_rate_bps.encode_size()
            + self.floating_spread_bps.encode_size()
    }
}
...
pub struct MarketState {
    ...
    pub interest_rate_terms: InterestRateTerms,
...
impl MarketState {
    pub fn new(
        ...
        interest_rate_terms: InterestRateTerms,
    ...
    pub fn interest_rate_quote_from_isfr(
        ...
        Ok(InterestRateQuote {
            benchmark: self.interest_rate_terms.benchmark,
            benchmark_epoch: snapshot.epoch,
            benchmark_timestamp: snapshot.timestamp,
            benchmark_rate_bps,
            fixed_rate_bps: self.interest_rate_terms.fixed_rate_bps,
            floating_spread_bps: self.interest_rate_terms.floating_spread_bps,
            floating_rate_bps: self
                .interest_rate_terms
                .floating_rate_bps(benchmark_rate_bps)?,
        })
    }
    ...
    pub async fn interest_rate_quote<P: IsfrRateProvider + Sync>(
        ...
        self.interest_rate_quote_from_isfr(&snapshot)
    }
}
...
impl Write for MarketState {
    fn write(&self, buf: &mut impl bytes::BufMut) {
        ...
        self.interest_rate_terms.write(buf);
...
impl Read for MarketState {
    ...
    fn read_cfg(buf: &mut impl bytes::Buf, _: &Self::Cfg) -> Result<Self, CodecError> {
        Ok(Self {
            ...
            interest_rate_terms: InterestRateTerms::read(buf)?,
...
impl EncodeSize for MarketState {
    fn encode_size(&self) -> usize {
        self.id.encode_size()
            ...
            + self.interest_rate_terms.encode_size()
        ...
        1 + match self {
            Self::CreateMarket {
                ...
                collateral_coin.encode_size()
                    ...
                    + interest_rate_terms.encode_size()
            }
            Self::SupplyCollateral { market_id, amount }
            | Self::WithdrawCollateral { market_id, amount }
            | Self::Borrow { market_id, amount }
...
mod tests {
    ...
    fn isfr_plus_200() -> InterestRateTerms {
        InterestRateTerms::isfr(600, 200)
    }
    ...
    fn signed_lending_lifecycle_tracks_balances_and_position() {
        deterministic::Runner::default().start(|context| async move {
            ...
            for tx in [
                Transaction::sign(
                    ...
                    LendingOperation::CreateMarket {
                        ...
                        interest_rate_terms: isfr_plus_200(),
                    },
                ),
                Transaction::sign(
                    &borrower_key,
            ...
            assert_eq!(
                market
                    .interest_rate_quote_from_isfr(&isfr_snapshot(450))
                    .unwrap(),
                InterestRateQuote {
                    benchmark: RateBenchmark::Isfr,
                    benchmark_epoch: 42,
                    ...
                    fixed_rate_bps: 600,
                    floating_spread_bps: 200,
                    floating_rate_bps: 650,
                }
            );
    ...
    fn rejects_borrow_above_collateral_factor() {
        deterministic::Runner::default().start(|context| async move {
            ...
            ) = seeded_state(context).await;
            {
                let mut lending = LendingLedger::new(&mut state);
                lending
                    .create_market(collateral_coin, borrow_coin, 5_000, isfr_plus_200())
                    .await
