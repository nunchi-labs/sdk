...
use nunchi_bridge::{BridgeHandle, SubmitResult};
        ...
        + 'static,
{
    module.register_async_method("bridge.status", |_raw, context, _| async move {
        ...
        let latest_foreign_view = context.node.bridge.latest().map(|f| f.view().get());
    ...
    module.register_async_method("bridge.submitFinalization", |raw, context, _| async move {
        ...
        let result = context.node.bridge.submit(finalization);
        let latest = context.node.bridge.latest();
    ...
    module.register_async_method("bridge.latestAccepted", |_raw, context, _| async move {
        RpcResult::Ok(context.node.bridge.latest().map(|f| encode_hex(&f)))
    })?;
