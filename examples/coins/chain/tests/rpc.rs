...
fn sort_pending(mut pending: Vec<Transaction>) -> Vec<Transaction> {
    pending.sort_by_key(Transaction::digest);
    pending
}
...
fn rpc_serves_status_and_filters_submissions_over_http() {
    tokio::Runner::default().start(|context| async move {
        ...
        assert_eq!(
            sort_pending(submitter.pending(usize::MAX).await),
            sort_pending(vec![transaction.clone().into(), perpetual.clone().into()])
        );
        ...
        assert_eq!(
            sort_pending(submitter.pending(usize::MAX).await),
            sort_pending(vec![transaction.into(), perpetual.into()])
        );
