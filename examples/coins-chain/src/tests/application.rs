...
use nunchi_common::{empty_receipts_root, QmdbBackend, QmdbBatch, QmdbDatabaseSet, QmdbState};
use nunchi_mempool::{Mempool, PoolConfig};
use std::sync::Arc;

use crate::application::{genesis_payload, BasicApplication};
...
fn proposal_skips_unregistered_multisig() {
    ...
    runner.start(|context| async move {
        ...
        assert_eq!(output.transactions[0].receipt.event_count, 1);
        assert_eq!(output.transactions[0].events.len(), 1);
        assert_eq!(output.transactions[0].events[0].module.as_ref(), b"coins");
        assert_eq!(
            output.transactions[0].events[0].kind.as_ref(),
            b"token_created"
        );
    });
}
