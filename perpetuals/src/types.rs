...
/// Derive a deterministic position id for one `(owner, market, side)` tuple.
pub fn derive_position_id_for_side(owner: &Address, market: &MarketId, side: Side) -> PositionId {
    let mut hasher = Sha256::new();
    hasher.update(super::PERPETUALS_NAMESPACE);
    hasher.update(b"/position-side/");
