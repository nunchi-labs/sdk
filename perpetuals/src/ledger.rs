use crate::{
    derive_market_id, derive_position_id, derive_position_id_for_side, Address, Authorization,
    Market, MarketId, OraclePricePayload, PerpetualDB, PerpetualOperation, Position, PositionId,
    Side, Transaction, BPS_DENOMINATOR, MAX_PRICE_DECIMALS, PERPETUALS_NAMESPACE, PRICE_SCALE,
};
...
impl<D: PerpetualDB + CoinDB + StateStore + Send + Sync> PerpetualLedger<D> {
    ...
    /// Apply a CLOB fill settlement for one party on a perpetuals market.
    pub async fn apply_fill_settlement(
        &mut self,
        owner: Address,
        market_id: MarketId,
        side: Side,
        ...
        let opposite_id = derive_position_id_for_side(&owner, &market_id, side.opposite());
        let mut remaining = scaled_quantity;
        if let Some(mut opposite) = self.db.position(&opposite_id).await? {
            let close_qty = opposite.quantity.min(remaining);
            if close_qty > 0 {
                self.apply_partial_close(&mut market, &mut opposite, close_qty, price, &owner)
                    .await?;
                remaining = remaining
    ...
    async fn apply_partial_close(
        ...
        let total_qty = position.quantity;
        let collateral_release = position
            .collateral
        ...
        let closed = Position {
            quantity: close_qty,
            collateral: collateral_release,
            ..position.clone()
        };
        let pnl = pnl(&closed, price)?;
        let mut payout = i128_from_u128(collateral_release)?;
        payout = payout
            .checked_add(pnl)
            .ok_or(PerpetualError::ArithmeticOverflow)?;
        if payout > 0 {
            let amount = u128::try_from(payout).map_err(|_| PerpetualError::ArithmeticOverflow)?;
            pay_from_escrow_and_insurance(&mut self.db, owner, market.collateral_asset, amount)
                .await?;
        }
        decrement_open_interest(market, &closed)?;
        ...
        position.collateral = position
            .collateral
            .checked_sub(collateral_release)
            .ok_or(PerpetualError::ArithmeticOverflow)?;
        if position.quantity == 0 {
            self.db.remove_position(&position.id);
        } else {
            self.db.set_position(position);
        }
        Ok(())
    ...
    async fn apply_same_side_fill(
        ...
        let position_id = derive_position_id_for_side(owner, &market_id, side);
        let lot_quantity = quantity
            .checked_div(PRICE_SCALE)
            .ok_or(PerpetualError::ArithmeticOverflow)?;
        let collateral = initial_margin_for_quantity(price, lot_quantity, market.max_leverage_bps)?;
        self.deposit_collateral(owner, market.collateral_asset, collateral)
            .await?;

        if let Some(mut position) = self.db.position(&position_id).await? {
            let total_qty = position
                .quantity
                .checked_add(quantity)
                .ok_or(PerpetualError::ArithmeticOverflow)?;
            let weighted_entry = position
                .entry_price
            ...
            position.collateral = position
                ...
                .ok_or(PerpetualError::ArithmeticOverflow)?;
            increment_open_interest(market, side, quantity)?;
            self.db.set_position(&position);
        } else {
            let position = Position {
                id: position_id,
    ...
    async fn apply_operation(
        ...
        match operation {
            ...
            PerpetualOperation::OpenPosition {
                ...
                #[cfg(not(feature = "mock-execution"))]
                {
                    let _ = (market, side, collateral, leverage_bps);
                    return Err(PerpetualError::Unauthorized);
                }
                #[cfg(feature = "mock-execution")]
                self.open_position(
...
fn initial_margin_for_quantity(
    ...
    // `quantity` is in CLOB lot units; notional uses native price * quantity.
    let notional = price
        .checked_mul(quantity)
        .ok_or(PerpetualError::ArithmeticOverflow)?;
    if notional == 0 {
    ...
    Ok(notional
        ...
        .ok_or(PerpetualError::ArithmeticOverflow)?
        / u128::from(max_leverage_bps))
}

/// Apply a CLOB fill settlement for one party on a perpetuals market.
pub async fn apply_fill_settlement<D>(
    db: &mut D,
    owner: Address,
    market_id: MarketId,
    side: Side,
    ...
    D: PerpetualDB + CoinDB + StateStore + Send + Sync,
{
    PerpetualLedger::new(db)
        .apply_fill_settlement(owner, market_id, side, price, quantity, context)
        .await
}
